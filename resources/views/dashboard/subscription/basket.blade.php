...
$PassLoopCount = "0";
$PassExamName = "-";
$User = DB::table('users')->select('u_fname','u_lname','email')->where('id', $u_id)->get();
$PassUserFName = $User->first()->u_fname;
$PassUserLName = $User->first()->u_lname;
$PassUserEmail = $User->first()->email;
@endphp
    ...
    <section class="content-panel">
        ...
        @if($Baskets->count() == 0)
            <div class="alert alert-info" role="alert" style="margin-top: 15px;"> 
                Your basket is empty. Please add items to your basket before proceeding to checkout.
            </div>
        @else
            <div class="alert alert-info" role="alert" style="margin-top: 15px;">
                You have {{$Baskets->count()}} item(s) in your basket. Please review your items before proceeding to checkout.
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            ...
                            @foreach ($Baskets as $Basket)
                                ...
                                <tr class='normal' >
                                    ...
                                    <td class='text-center' style='width: 130px;'>
                                        @php
                                            ...
                                            if($Basket->ba_discount_price == '0.00')
                                                ...
                                                {
                                                    $ExamTotal = $ExamTotal + $Basket->ba_discount_price;
                                    ...
                                    @if($Basket->ba_discount_price == '0.00')
                                        <td class='text-center'>{{ number_format($ExamPrice, 2, '.', '')}}</td>
                                        <td class='text-center'>{{ number_format($ExamPrice, 2, '.', '')}}</td>
                                    @else
                                        <td class='text-center'><s>{{ number_format($ExamPrice, 2, '.', '')}}</s><br>{{ number_format($Basket->ba_discount_price, 2, '.', '')}}</td>
                                        <td class='text-center'>{{ number_format($Basket->ba_discount_price, 2, '.', '')}}</td>
                                    @endif
                                </tr>
                            @endforeach
                            ...
                            <tr class='info'>
                                ...
                                <th class='text-center'>{{ number_format($ExamTotal, 2, '.', '')}}</th>
                            </tr>
                            <tr class='normal'>
                                ...
                                <th class='text-center text-dark text-opacity-25'>$ {{ number_format($TotalDiscount, 2, '.', '')}} AUD</th>
                            </tr>
                            <tr class='danger'>
                                <th colspan='5' class='text-right pr-5'>Total</th>
                                <th class='text-center'>$ {{ number_format($ExamTotal, 2, '.', '')}} AUD</th>
                            </tr>
                        </tbody>
                    </table>    
                    </form>    
                    <div style='text-align:center;'>
                        <form class="paypal" action="{{ route('basketdummypay') }}" method="post" >
                            @csrf
                            ...
                            <input type="hidden" name="custom" value="{{$u_id}}" / >
                            <input type="hidden" name="paypal_my_proname" value="{{$PassExamName}}" / >
                            <input type="hidden" name="paypal_my_TotalPrice" value="{{$ExamTotal}}" / >                    
                            <input type="hidden" name="first_name" value="{{$PassUserFName}}"  />
                            <input type="hidden" name="last_name" value="{{$PassUserLName}}"  />
                            <input type="hidden" name="payer_email" value="{{$PassUserEmail}}"  />
                            <input type="hidden" name="item_number" value="{{$PassLoopCount}}" / > 
                            <input type="submit" class='btn btn-success checkout-button' name="submit" value="Checkout Dummy"/>
                        </form>
